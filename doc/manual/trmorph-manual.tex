...
\xmpl{evdekilerinki}.\footnote{Rough translation: `the \emph{ones} that
...
When there is no surface suffix, a zero-derivation tag \mtag[def]{0} is
inserted before the POS tag.

The example in (\ref{ig-example}) is also interesting because of the
fact the suffix \sffx{ki} may result indefinitely long words (see
Section~\ref{ssec:ki}).
...
The reflexive pronoun \xmpl{kendi(si)} is marked as
\mtag[def]{Prn:pers:refl}.
...
\emph{dative} and \emph{instrumental} cases are marked 
\mtag[def]{Postp:ablC},
\mtag[def]{Postp:datC},
and \mtag[def]{Postp:insC} respectively. Postpositions that require
non-case marked complement are tagged \mtag[def]{Postp:nomC}. The
...
Finally, postpositions that require numeric expressions as their
...
class of nominals. Most adverbs and adjectives can function as nouns (or
...
\emph{zero derivation}. A zero derivation is always marked with the tag
...
Possessive markers follow either the nominal stem, or the plural
marker. Besides marking for possession, these suffixes have a number
of other functions, such as deriving pronouns when attached to
adjectivals (e.g., determiners or adjectives).

The first five suffixes in Table~\ref{tbl:noun-inflections} are commonly
...
noun compounds. One of types of noun compounds in Turkish is formed by
a bare noun and a (head) noun with suffix \sffx{(s)I}, for example
\xmplt{at araba\emph{sı}}{horse carriage}.\footnote{Even though one
can assume that this use is somewhat related to possession, it is not
strictly possessive marking (the horse does not own the carriage).
Furthermore, although the surface suffix is skipped after a compound
marker, a compound of this form may also be in possessive form (e.g.,
`someone's horse carriage').} TRmorph marks this usage of the suffix
\sffx{(s)I} as \mtag[def]{ncomp}. In this use, this suffix always
causes ambiguities.  Besides the fact that a noun suffixed with
\sffx{(s)I} can either be marked for possession or as the head of a noun
compound, since one of the two \sffx{(s)I} suffixes following each
other is deleted from the surface form, it can also be both (a noun
compound marked for possession).
...
rarely exceed 2 in practice, there is no principled limit.  As a
result, length of a Turkish word is unbounded in-principle.
...
verb by a zero derivation, and then marking it with the
...
set of suffixes that form compound verbs. These suffixes are related
to some stand-alone verbs. These suffixes are
...
\xmplt{çıkıverebilir}{he/sh/it may possibly come out/show up} analyzed
as `çık\mtag{V}\mtag{iver}\mtag{abil}\mtag{aor}\mtag{3s}'.
...
Currently, these suffixes are not marked as derivations (they do not
cause an IG change).
...
agreement markers. The surface form of the person-number agreement markers
...
\subsection{\label{ssec:qestion}The question particle}
...
agreement marker \mtag{3s} or \mtag{3p}, since a predicate with null
...
rules, and the underspecified vowel on \sffx{mI} is realized based on
...
altogether. In this case the lowercase tag \mtag[def]{q} is used.
...
of the TAM markers, namely \mtag{aor}, \mtag{evid}, \mtag{fut},
\mtag{makta} and less commonly \mtag{cont}, derive adjectivals
...
\xmpl{görülmüş} in \xmplt{görülmüş mektup}{see-PASV-EVID letter = the
letter that was seen} is analyzed as
`gör\mtag{V}\mtag{pass}\mtag{V}\mtag{evid}\mtag{Adj}'.
...
whose remainder \sffx{r} also matches a verbal suffix (aorist).
...
\mtag[def]{ca}   & \sffx{CA}     & NA AA JJ MJ \\ % \mtag{N}{Adv} \mtag{Adv}{Adv} \mtag{Adj}{Adj} \mtag{Num}{Adj} \\ 
...
\item Ambiguous root forms, for example \emph{yüz} can be analyzed as:
    \begin{enumerate}
    \item yüz\mtag{N} `face'
    \item yüz\mtag{Num} `hundred'
    \item yüz\mtag{V}\mtag{imp}\mtag{2s} `swim'
    \end{enumerate}
...
suffixes, for example \emph{buna} can be analyzed as
    \begin{enumerate}
    \item bu\mtag{Prn:dem}\mtag{dat} `this-DAT'
    \item buna\mtag{V}\mtag{imp}\mtag{2s} `become senile-IMP'
    \item bun\mtag{N}\mtag{dat}   `trouble-DAT'
    \end{enumerate}
...
stages disambiguate between them.
...
\item\label{itm:sub-suffix} The surface form of a suffix is a combination of two other suffixes. For example, the word \xmpl{evleri} can be
    \begin{enumerate}
    \item ev\sffx{leri} `ev\mtag{N}\mtag{p3p} = their house'
    \item ev\sffx{ler}\sffx{i} `ev\mtag{N}\mtag{pl}\mtag{acc} = houses-ACC'
    \end{enumerate}
Furthermore, the same word can also be analyzed as 
    \begin{enumerate}
    \item `ev\mtag{N}\mtag{pl}\mtag{p3s}' 
    \item `ev\mtag{N}\mtag{pl}\mtag{p3p}'  
    \item `ev\mtag{N}\mtag{ncomp}\mtag{p3p}'
    \item `ev\mtag{N}\mtag{ncomp}\mtag{pl}'
    \item `ev\mtag{N}\mtag{ncomp}\mtag{pl}\mtag{p3p}'
    \item `ev\mtag{N}\mtag{ncomp}\mtag{pl}\mtag{p3s}' 
    \item `ev\mtag{N}\mtag{ncomp}\mtag{pl}\mtag{p3p}'
    \end{enumerate}
...
\item An analysis with multiple morphemes is also a (derived) lexicalized form. For example the word \xmpl{konuşma} can be analyzed as 
    \begin{enumerate}
    \item konuşma\mtag{N} `speech'
    \item konuş\mtag{V}\mtag{vn:inf}\mtag{N}  infinitive `to speak', e.g., as in \xmplt{konuşmamızı isemiyorlar}{The do not want us \emph{to speak}}
    \item konuş\mtag{V}\mtag{neg}\mtag{imp}\mtag{2s} `speak-NEG-IMP = don't talk'
    \end{enumerate}
\item\label{itm:same-surface} different affixes surfacing the same way, \emph{evin} can be
    \begin{enumerate}
    \item ev\sffx{(n)In} `ev\mtag{N}\mtag{gen} =of the house'
    \item ev\sffx{(I)n}  `ev\mtag{N}\mtag{p2s} =your house'
    \end{enumerate}
\item\label{itm:different-func} The same surface suffix has multiple
functions. For example, the word \xmpl{doktorlar} can be,
    \begin{enumerate}
    \item doktor\mtag{N}\mtag{pl} `doctors'
    \item doktor\mtag{N}\mtag{0}\mtag{V}\mtag{cpl:pres}\mtag{3p} `they are doctors'
    \end{enumerate}
\item\label{itm:null-suffix} Some suffixes are not realized on the surface in the neighborhood of some other suffixes. These are generally, but not always, the suffixes having the same or similar surface forms. For example, \xmpl{evleri} (the example in item \ref{itm:sub-suffix}) may be analyzed as
    \begin{enumerate}
    \item ev\mtag{N}\mtag{p3p}  as in \xmplt{Annem ve babamın evleri Istanbul'da}{My parents' house is in Istanbul}
    \item ev\mtag{N}\mtag{pl}\mtag{p3p} as in \xmplt{Annem ve babamın bütün evleri deniz manzaralı}{All houses of my parents have a see view}.
    \end{enumerate}
...
\begin{enumerate}
\item[(c)] araba\mtag{N}\mtag{siz}\mtag{Adj} `without a car'
\item[(d)] araba\mtag{N}\mtag{p3s}\mtag{siz}\mtag{Adv} `without a his/her car'
\item[(e)] araba\mtag{N}\mtag{ncomp}\mtag{siz}\mtag{Adv}, e.g., in \xmplt{at arabasız}{without a horse carriage}
\item[(f)] araba\mtag{N}\mtag{ncomp}\mtag{p3s}\mtag{siz}\mtag{Adv}, e.g., in \xmplt{at arabasız}{without his/her horse carriage}
\end{enumerate}
...
`sen\mtag{Prn:pers:1s}\mtag{ins}' should be \xmpl{sen-in-le} where the
suffix \sffx{in} is redundant. Some speakers tend not to use \sffx{in}
int such constructions. TRmorph accepts both use, hence the generation
will be ambiguous.

\item Some borrowed words include a few vowels with circumflex, namely
\xmpl{â},
\xmpl{û} and
\xmpl{î}. Except for a few words where use of circumflex help
